...
write_results <- function(results, path = "MPTmultiverse_"
  ...
  readr::write_csv(tidyr::unnest(results, est_group), 
                   path = paste0(path,"est_group.csv"))
  readr::write_csv(tidyr::unnest(results, est_indiv), 
                   path = paste0(path,"est_indiv.csv"))
  readr::write_csv(tidyr::unnest(results, est_rho), 
                   paste0(path,"est_rho.csv"))
  
  readr::write_csv(tidyr::unnest(results, test_between), 
                   paste0(path,"test_between.csv"))
  
  readr::write_csv(tidyr::unnest(results, gof), 
                   paste0(path,"gof.csv"))
  readr::write_csv(tidyr::unnest(results, gof_indiv), 
                   paste0(path,"gof_indiv.csv"))
  readr::write_csv(tidyr::unnest(results, gof_group), 
                   paste0(path,"gof_group.csv"))
  
  readr::write_csv(tidyr::unnest(results, fungibility), 
