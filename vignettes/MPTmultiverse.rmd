...
res_treebugs <- purrr::map(c("simple", "simple_pooling", "trait", "beta", "trait_uncorrelated"),
                    mpt_treebugs_safe, 
                    dataset = DATA_FILE, data = data, model = EQN_FILE,
                    col_id = COL_ID, col_condition = COL_CONDITION)

results <- dplyr::bind_rows(res_mptinr, res_treebugs)
```
...
results <- res_mptinr
class(results) <- c("multiverseMPT", class(results))
plot(results, save = FALSE)
```
